...
export function TaskDetailView({ task, post, related, comments = [] }: { task: TaskKey; post: SitePost; related: SitePost[]; comments?: Array<{ id: string; name: string; comment: string; createdAt: string }> }) {
  const detailVars = { '--detail-bg': '#f4f4f4', '--detail-text': '#101828', '--detail-surface': '#ffffff', '--detail-accent': '#0b6fdc' } as CSSProperties
...
function BackLink({ task }: { task: TaskKey }) {
  ...
  return (
    <Link href={taskConfig?.route || '/'} className="inline-flex items-center gap-2 rounded-md border border-[var(--editable-border)] bg-white px-4 py-2 text-sm font-black">
  ...
  return (
    <section className="mx-auto max-w-[var(--editable-container)] px-4 py-10 sm:px-6 lg:px-8">
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-[minmax(0,1fr)_420px]">
        <article className="rounded-lg border border-[var(--editable-border)] bg-white p-6 shadow-sm sm:p-8">
          <div className="grid gap-6 sm:grid-cols-[150px_1fr]">
            <div className="flex h-36 w-36 items-center justify-center overflow-hidden rounded-lg bg-[#f8fbff] ring-1 ring-[var(--editable-border)]">
              {logo ? <img src={logo} alt="" className="h-full w-full object-cover" /> : <Building2 className="h-14 w-14 opacity-40" />}
            </div>
            <div>
              <p className="text-xs font-black uppercase tracking-[0.18em] text-[var(--detail-accent)]">Business listing</p>
              <h1 className="mt-3 text-4xl font-black leading-tight tracking-tight sm:text-5xl">{post.title}</h1>
             
              <div className="mt-5 flex flex-wrap gap-2">
                <span className="rounded-md bg-[#e8f2ff] px-3 py-1.5 text-xs font-black text-[#0b6fdc]">Verified profile</span>
                <span className="rounded-md bg-[#fff1ed] px-3 py-1.5 text-xs font-black text-[#d9381e]">Quick contact ready</span>
              </div>
            </div>
          </div>
