...
const fieldClass = 'rounded-md border border-slate-300 bg-white px-4 py-3 text-sm font-bold text-slate-950 outline-none transition placeholder:text-slate-400 focus:border-[#0b6fdc]'
...
export default function CreatePage() {
  ...
  const enabledTasks = useMemo(() => {
    const listing = SITE_CONFIG.tasks.find((item) => item.enabled && item.key === 'listing')
    return listing ? [listing] : SITE_CONFIG.tasks.filter((item) => item.enabled)
  }, [])
  ...
  if (!session) {
    return (
      <EditableSiteShell>
        <main className="min-h-screen bg-[#f4f4f4] px-4 py-12 text-slate-950 sm:px-6 lg:px-8">
          <section className="mx-auto grid max-w-[980px] gap-8 rounded-lg border border-black/10 bg-white p-7 shadow-sm md:grid-cols-[0.9fr_1.1fr] md:p-8">
            <div className="flex h-full min-h-72 items-center justify-center rounded-lg bg-[#0b2d57] text-white">
            ...
            <div className="self-center">
              ...
              <div className="mt-8 flex flex-wrap gap-3">
                <Link href="/login" className="inline-flex items-center gap-2 rounded-md bg-[#0b6fdc] px-6 py-3 text-sm font-black text-white">Login <ArrowRight className="h-4 w-4" /></Link>
    ...
    <EditableSiteShell>
      <main className="min-h-screen bg-[#f4f4f4] text-slate-950">
        <section className="mx-auto max-w-[1180px] px-4 py-10 sm:px-6 lg:px-8">
          <div className="grid gap-8 rounded-lg border border-black/10 bg-white p-6 shadow-sm lg:grid-cols-[0.85fr_1.15fr] lg:p-8">
            <aside>
              ...
              <div className="mt-8 grid gap-3 sm:grid-cols-2">
                {enabledTasks.map((item) => {
                  ...
                  return (
                    <button key={item.key} type="button" onClick={() => setTask(item.key)} className={`rounded-lg border p-4 text-left transition ${active ? 'border-[#0b6fdc] bg-[#e8f2ff] text-slate-950' : 'border-black/10 bg-white hover:-translate-y-0.5'}`}>
            ...
            <form onSubmit={submit} className="rounded-lg border border-black/10 bg-[#f8fbff] p-5 sm:p-7">
              ...
              <div className="mt-6 grid gap-4">
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  ...
                  <input className={fieldClass} value={url} onChange={(event) => setUrl(event.target.value)} placeholder="Website or booking URL" />
                </div>
                ...
                <textarea className={`${fieldClass} min-h-48`} value={body} onChange={(event) => setBody(event.target.value)} placeholder="Services, address, opening hours, contact details, and customer notes" required />
              </div>
              ...
              <button type="submit" className="mt-5 inline-flex h-12 w-full items-center justify-center gap-2 rounded-md bg-[#0b6fdc] px-6 text-sm font-black uppercase tracking-[0.16em] text-white transition hover:-translate-y-0.5">
